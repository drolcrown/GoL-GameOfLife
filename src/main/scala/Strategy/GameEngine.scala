...
/**
  ...
  */
class GameEngine extends DerivationStrategy {
  ...
  protected def validPosition(i: Int, j: Int) =
  ...
  /**
    ...
    * @throws IllegalArgumentException caso a posicao (i, j) nao seja valida.
    */
  @throws(classOf[IllegalArgumentException])
  protected def makeCellAlive(i: Int, j: Int) = {
  ...
  /**
    ...
    */
  @throws(classOf[IllegalArgumentException])
  protected def isCellAlive(i: Int, j: Int): Boolean = {
    if (validPosition(i, j)) {
      cells(i)(j).isAlive
    } else {
      throw new IllegalArgumentException
  ...
  /**
    ...
    */
  protected def numberOfAliveCells {
    var aliveCells = 0
    for (i <- 0 until height) {
      for (j <- 0 until width) {
  ...
  override def shouldRevive(line: Int, column: Int) : Boolean = {
    if (!isCellAlive(line, column) && numberOfNeighborhoodAliveCells(line, column) == 3)
      true
    else
      false
  }
